...
/*
- v1.31
	- Class descriptions get cut off. Fix it.
	- Make command to get information of class.
	- Make command to get information of wearables.
	- Repurpose sm_info so it only shows the information about the current weapon.
*/
...
ConVar g_bDisplayInfoOnSpawn; // Convar that displays information to the player about the class/weapons.
...
bool g_bDontShowWeaponHelpToggle[MAXPLAYERS+1] = false; // Bool that indicates if the weapon info should show up on spawn.
Handle g_hWeaponHelpToggleCookie = INVALID_HANDLE; // Cookie that handles automatic weapon info showing up.
...
char g_cRebalance_ItemDescription[MAXIMUM_DESCRIPTIONLIMIT][MAXIMUM_ADDITIONS];
...
char g_cRebalance_ClassDescription[MAXIMUM_DESCRIPTIONLIMIT][view_as<int>(TFClassType)];
...
public void OnPluginStart()
{
	...
	g_bDisplayInfoOnSpawn = CreateConVar("sm_tfbalance_infoonspawn", "1",
	"Displays weapons' information on spawn. Default = 1, 0 to disable.", FCVAR_DONTRECORD|FCVAR_PROTECTED);
	g_bFirstTimeInfoOnSpawn = CreateConVar("sm_tfbalance_firsttimeinfo", "1",
	...
	// Info on spawn cookie:
	g_hWeaponHelpToggleCookie = RegClientCookie("weaponhelptoggle", "Remembers weapon help visibility for the player", CookieAccess_Private);
	...
	RegConsoleCmd("sm_info", WeaponHelp, "Displays info for the rebalanced weapons");
	RegConsoleCmd("sm_information", WeaponHelp, "Displays info for the rebalanced weapons");
	RegConsoleCmd("sm_changes", WeaponHelp, "Displays info for the rebalanced weapons");
	
	// Class information command:
	RegConsoleCmd("sm_info_class", ClassHelp, "Displays info for the rebalanced weapons");
	RegConsoleCmd("sm_information_class", ClassHelp, "Displays info for the rebalanced weapons");
	RegConsoleCmd("sm_changes_class", ClassHelp, "Displays info for the rebalanced weapons");
	
	// Wearable information command:
	RegConsoleCmd("sm_info_wearables", WearableHelp, "Displays info for the rebalanced weapons");
	RegConsoleCmd("sm_information_wearables", WearableHelp, "Displays info for the rebalanced weapons");
	RegConsoleCmd("sm_changes_wearables", WearableHelp, "Displays info for the rebalanced weapons");
	
	// Weapon automatic info toggle command:
	RegConsoleCmd("sm_infotoggle", WeaponHelpToggle, "Toggles info for the rebalanced weapons whenever you spawn");
	RegConsoleCmd("sm_informationtoggle", WeaponHelpToggle, "Toggles info for the rebalanced weapons whenever you spawn");
	RegConsoleCmd("sm_changestoggle", WeaponHelpToggle, "Toggles info for the rebalanced weapons whenever you spawn");
}
...
public void OnMapStart()
{
	...
	{
		PrintToServer("[TFRebalance] Stored %i weapons and %i classes in total to change.",
	...
	if (g_bLogMissingDependencies.BoolValue) // That is, if this convar is set to true.
	{
		if (!g_bIsTF2AttributesEnabled)
			LogMessage("[TFRebalance] tf2attributes is not loaded. "
			... "This will prevent the plugin from adding attributes on classes.");
	}
}
...
public Action WeaponHelp(int iClient, int iArgs)
{
	...
	g_bEnablePlugin.BoolValue)
		CreateBalanceMenu(iClient, WeaponHelp);
}
...
public Action WeaponHelp(int iClient, int iArgs)
{
	if (IsValidClient(iClient) && IsPlayerAlive(iClient) &&
	g_bEnablePlugin.BoolValue)
		CreateBalanceMenu(iClient, ClassHelp);
}
...
public Action WeaponHelp(int iClient, int iArgs)
{
	if (IsValidClient(iClient) && IsPlayerAlive(iClient) &&
	g_bEnablePlugin.BoolValue)
		CreateBalanceMenu(iClient, WearableHelp);
}

// Command that toggles said information if given automatically by the player.
public Action WeaponHelpToggle(int iClient, int iArgs)
{
	if (IsValidClient(iClient) && g_bEnablePlugin.BoolValue)
	{
		...
		if (g_bDontShowWeaponHelpToggle[iClient])
		{
			g_bDontShowWeaponHelpToggle[iClient] = false;
			CPrintToChat(iClient, "{community}Help will now display on spawn.");
		}
		else if (!g_bDontShowWeaponHelpToggle[iClient])
		{
			g_bDontShowWeaponHelpToggle[iClient] = true;
			CPrintToChat(iClient, "{community}Help will not display on spawn.");
		}
		
		Format(cPreference, sizeof(cPreference), "%i", g_bDontShowWeaponHelpToggle[iClient]);
		SetClientCookie(iClient, g_hWeaponHelpToggleCookie, cPreference);
	}
	else CPrintToChat(iClient, "{community}You are an invalid client, sorry!");
}

// We get the cookie of if the client wanted info shown on spawn or not.
public void OnClientCookiesCached(int iClient)
{
	char cValue[8];
	if (g_hWeaponHelpToggleCookie != INVALID_HANDLE)
	{
		GetClientCookie(iClient, g_hWeaponHelpToggleCookie, cValue, sizeof(cValue));
		
		int iValue = StringToInt(cValue);
		g_bDontShowWeaponHelpToggle[iClient] = view_as<bool>(iValue);
	}
}
...
public Action Event_PlayerSpawn(Handle hEvent, const char[] cName, bool dontBroadcast)
{
	...
	{	
		...
		if (!g_bFirstSpawn[iClient] && g_bFirstTimeInfoOnSpawn.BoolValue)
		{
			...
			... "Type /info to learn more about what changed on your weapons (and yourself), "
			... "or type /infotoggle to hide the menu on spawn automatically.");
		...
		if (g_bDisplayInfoOnSpawn.BoolValue && !g_bDontShowWeaponHelpToggle[iClient]) CreateBalanceMenu(iClient);
	}
}
...
public Action CreateBalanceMenu(int iClient, HelpType htTypeOfHelp)
{
	...
	int iWeapon = -1, iWeaponIndex = -1, iWeaponSlots;
	
	// If the client is valid and he's alive...
	if (IsValidClient(iClient) && IsPlayerAlive(iClient))
	{
		// We get the descriptions of weapons.
		while (iWeaponSlots <= TFWeaponSlot_Item2) // Item2 slot is the last slot.
		{
			//PrintToServer("Checking slot %i", iWeaponSlots);
			// We get the weapon in such slot.
			iWeapon = GetPlayerWeaponSlot(iClient, iWeaponSlots);
			if (iWeapon != -1)
			{
				//PrintToServer("Checking index on %i", iWeapon);
				iWeaponIndex = GetEntProp(iWeapon, Prop_Send, "m_iItemDefinitionIndex");
				
				if (iWeaponIndex != -1)
				{
					//PrintToServer("Checking if description exists on %i", iWeaponIndex);
			...
			iWeaponSlots++, iWeapon = -1, iWeaponIndex = -1;
		}
		
		// We get the description of class.
		if (htTypeOfHelp == ClassHelp)
		{
			TFClassType tfClass = TF2_GetPlayerClass(iClient);
			if (g_bRebalance_ClassChanged[tfClass])
			{
				StrCat(cChanges, sizeof(cChanges),
				g_cRebalance_ClassDescription[view_as<int>(tfClass)]);
				// Turns newlines on the keyvalues file into REAL newlines
				ReplaceString(cChanges, sizeof(cChanges), "\\n", "\n");
		...
		// We get the descriptions of wearables.
		int WearableItem = -1;
	...
	if (!StrEqual(cChanges, "Here's the changes:\n", false))
	{
	...
	do
	{
		...
		if (StrEqual("classes", cIDWeaponIndex, false)) // Classes section.
		{
			...
			do
			{
				...
				g_cRebalance_ClassDescription[view_as<int>(tfClassModified)],
				sizeof(g_cRebalance_ClassDescription));
				//PrintToServer("Class Description:\n%s",
		...
		{
			if (!StrEqual(cIDsOfWeapons[iWeaponCountWithinString], "", false) &&
			!StrEqual(cIDsOfWeapons[iWeaponCountWithinString], "", false)) // If the char is empty, then we'll stop. But if not...
			{
				...
				if (iIDWeaponIndex != -1) // If a weapon ID is defined
				{
					...
					KvGetString(g_hKeyvaluesAttributesFile, "info",
					g_cRebalance_ItemDescription[g_iRebalance_ItemIndexChangesNumber],
					sizeof(g_cRebalance_ItemDescription));
					//PrintToServer("Weapon Index: %i for %i",
					//g_iRebalance_ItemIndexChangesNumber, iIDWeaponIndex);
					...
					do
					{
						...
						if (StrEqual(cAttributeAddition, cAttributeString, false)) // Adding an attribute - gets the id and value inside the attribute section.
						{
							...
							//g_iRebalance_ItemAttribute_Add[g_iRebalance_ItemIndexChangesNumber][Rebalance_ItemAttribute_AddNumber[g_iRebalance_ItemIndexChangesNumber]], 
							//Rebalance_ItemAttribute_AddValue[g_iRebalance_ItemIndexChangesNumber][Rebalance_ItemAttribute_AddNumber[g_iRebalance_ItemIndexChangesNumber]]);
							//PrintToServer("%i - %i", g_iRebalance_ItemIndexChangesNumber, Rebalance_ItemAttribute_AddNumber[g_iRebalance_ItemIndexChangesNumber]);
						}
					}
					while (KvGotoNextKey(g_hKeyvaluesAttributesFile, false)); // Moves between attributes.
					...
					//iIDWeaponIndex,
					//Rebalance_ItemAttribute_AddNumber[g_iRebalance_ItemIndexChangesNumber]);
				}
			}
