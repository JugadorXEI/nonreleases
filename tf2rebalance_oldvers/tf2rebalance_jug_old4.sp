	...
	do
	{
		...
		if (StrEqual("classes", cIDWeaponIndex, false)) // Classes section.
		{
			...
			do
			{
				...
				{
					// If we had an info key before the attributes, we tell it to go to the next key.
					g_bRebalance_DoesClassHaveDescription[tfClassModified] = true;
					KvGotoNextKey(g_hKeyvaluesAttributesFile, false);
				}
				
				// We setup a search int for the setup attributes
				int iSearchAttributesInFile = 1;
				
				// KvGotoFirstSubKey(g_hKeyvaluesAttributesFile, true); // This should be an attribute[number] subkey.
